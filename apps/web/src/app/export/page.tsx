...
export default function ExportPage() {
  ...
  const preview = useMemo(() => buildSheetPreview(project, effectiveSettings, thematicMap, activeAnalysisLabel, activeScenarioId), [project, effectiveSettings, thematicMap, activeAnalysisLabel, activeScenarioId]);
  ...
  return (
    <main className="relative h-full overflow-hidden bg-[#F8FAFC] text-[#0F172A]">
      ...
      <section className="relative z-10 grid h-full min-h-0 grid-cols-[300px_minmax(620px,1fr)_300px] max-xl:grid-cols-[280px_minmax(0,1fr)] max-lg:grid-cols-1 max-lg:overflow-y-auto" style={{ gap: 12, padding: 12 }}>
        <LeftPanel settings={effectiveSettings} update={update} project={project} onAddViewport={addViewport} analysis={analysis} analysisLabel={activeAnalysisLabel} activeScenarioId={activeScenarioId} />
...
function LeftPanel({ settings, update, project, onAddViewport, analysis, analysisLabel, activeScenarioId }: { settings: SheetSettings; update: <K extends keyof SheetSettings>(key: K, value: SheetSettings[K]) => void; project: FormiqProjectData; onAddViewport: (kind: ViewportKind) => void; analysis: AnalysisResult; analysisLabel: string; activeScenarioId: string }) {
  return <aside className="min-h-0 overflow-y-auto rounded-[20px] border border-white/70 bg-white/[0.62] p-5 backdrop-blur-3xl [scrollbar-width:thin]">
    ...
    <section className="mt-4 rounded-[14px] border border-[#E2E8F0] bg-white/[0.42] p-3"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">Источник анализа</h3><span className="h-2 w-2 rounded-full bg-[#22C55E]" title="Синхронизировано" /></div><p className="mt-2 text-[13px] font-medium">{analysisLabel}</p><p className="mt-1 text-[12px] text-[#64748B]">Сценарий: {activeScenarioId === "base" ? "Базовый" : activeScenarioId}</p><div className="mt-3 grid grid-cols-2 gap-2 text-[12px]"><MetricValue label="FAR" value={formatNumber(analysis.buildings.totalFloorArea / Math.max(analysis.territory.area, 1))} /><MetricValue label="GSI" value={`${formatNumber(analysis.buildings.footprintPercent)}%`} /><MetricValue label="Зданий" value={String(analysis.buildings.count)} /><MetricValue label="Озеленение" value={`${formatNumber(analysis.vegetation.territoryPercent)}%`} /></div></section>
...
function buildSheetPreview(project: FormiqProjectData, settings: SheetSettings, thematicMap: ThematicMapDefinition | null, analysisLabel: string, scenarioId: string) { const paper = getPaper(settings.paper, settings.orientation); const width = 1120; const height = Math.round(width * (paper.height / paper.width)); const margin = Math.round((settings.marginMm / paper.width) * width); const mapBox = { x: margin + 26, y: margin + 84, width: width - margin * 2 - 260, height: height - margin * 2 - 190 }; const shapes = collectPreviewShapes(project, mapBox, thematicMap); const grid = settings.showGrid ? createGrid(width, height, margin) : ""; const legend = settings.showLegend ? createLegend(width - margin - 170, margin + 260, thematicMap, analysisLabel) : ""; const north = settings.showNorth ? createNorthArrow(width - margin - 70, margin + 48) : ""; const scale = settings.showScale ? createScaleBar(margin + 40, height - margin - 88, settings.scale) : ""; const frame = settings.showFrame ? `<rect x="${margin}" y="${margin}" width="${width - margin * 2}" height="${height - margin * 2}" fill="none" stroke="#0F172A" stroke-width="1.2"/>` : ""; const labels = settings.showLabels ? createTitleBlock(width - margin - 300, height - margin - 104, settings, project) : ""; return { svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="100%" height="100%" role="img" aria-label="Лист компоновки FORMIQ"><rect width="${width}" height="${height}" fill="#fff"/>${grid}<rect x="${margin + 14}" y="${margin + 14}" width="${width - margin * 2 - 28}" height="${height - margin * 2 - 28}" fill="none" stroke="#CBD5E1" stroke-width="1" stroke-dasharray="5 5"/>${frame}<text x="${margin + 44}" y="${margin + 60}" font-family="Inter,Arial" font-size="30" font-weight="700" fill="#0F172A">${escapeXml(settings.title.toUpperCase())}</text><text x="${margin + 44}" y="${margin + 88}" font-family="Inter,Arial" font-size="16" fill="#64748B">${escapeXml(settings.subtitle)} · ${escapeXml(scenarioId === "base" ? "Базовый сценарий" : scenarioId)}</text><g opacity="0.85"><rect x="${mapBox.x}" y="${mapBox.y}" width="${mapBox.width}" height="${mapBox.height}" fill="#F8FAFC"/>${createBasePlan(mapBox)}${shapes}</g>${legend}${north}${scale}${labels}</svg>` }; }
...
function projectPoint(position: Array<number>, bounds: { west: number; south: number; east: number; north: number }, box: { x: number; y: number; width: number; height: number }): [number, number] { return [Math.round(box.x + ((position[0] - bounds.west) / Math.max(bounds.east - bounds.west, .000001)) * box.width), Math.round(box.y + (1 - (position[1] - bounds.south) / Math.max(bounds.north - bounds.south, .000001)) * box.height)]; }

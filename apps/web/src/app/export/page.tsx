...
function createMetricStrip(x: number, y: number, width: number, model: ReturnType<typeof buildAnalysisModel>, projection: ReturnType<typeof projectScenario>) { const items = [["FAR", formatNumber(projection.far)], ["GSI", formatNumber(projection.gsi / 100)], ["KPI", `${projection.score}/100`], ["Зданий", String(model.buildingCount)], ["Озеленение", model.metricsById.green.value]]; const itemWidth = Math.max(88, Math.floor(width / items.length)); return `<g>${items.map(([label, value], index) => { const itemX = x + index * itemWidth; return `<g><text x="${itemX}" y="${y}" font-family="Inter,Arial" font-size="10" fill="#64748B">${label}</text><text x="${itemX}" y="${y + 18}" font-family="Inter,Arial" font-size="16" font-weight="700" fill="#0F172A">${escapeXml(value)}</text></g>`; }).join("")}</g>`; }
